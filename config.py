...
def get_hf_text_config(config: PretrainedConfig):
    ...
    if hasattr(config, "text_config"):
        ...
        # Accept both dicts and config objects
        if isinstance(text_config, dict):
            if "num_attention_heads" not in text_config:
                raise ValueError("text_config dict missing 'num_attention_heads'")
            # Optionally convert to PretrainedConfig for downstream code
            from transformers import PretrainedConfig
            text_config = PretrainedConfig(**text_config)
            # Assign back so future calls use the fixed object
            config.text_config = text_config
        elif not hasattr(text_config, "num_attention_heads"):
            raise ValueError("text_config object missing 'num_attention_heads'")
        return text_config
    else:
        return config
